...
S2RDFQueryTranslator = "./QueryTrans_Short.jar"
...
def translateQuery(fileName, sqlDir):    
    ...
    if ((not "~" in fileName) and (not "rdf3x" in fileName)):
        ...
        if not os.path.exists(sqlDir+"/VP"):
          os.makedirs(sqlDir+"/VP")
        for case in tableSets.split("_"):
            args = parseArgs(case)
						# Aenderungen Max & Timo					
            case = case.upper()
            if (case=='VP'):
              status = executeCommand(fileName, sqlDir+"/"+case, dataSetName, testName+"--"+case, args)
            else:
              status = executeCommand(fileName, sqlDir, dataSetName, testName+"--"+case, args)
        subprocess.call("rm -f "+sqlDir+"/*.log", shell=True)
        subprocess.call("rm -f "+sqlDir+"/VP"+"/*.log", shell=True)
def main(argv):
    sparqlDir = ''
    sqlDir = ''
    global scaleUB
    try:
      opts, args = getopt.getopt(argv,"h:s:t:u:",["sdir=","tdir=", "scaleUB"])
    except getopt.GetoptError:
      print 'translateWatDivQueries.py -s <sparqlDir> -t <sqlDir> -u <scaleUB> err'
      sys.exit(2)
    for opt, arg in opts:
      if opt == '-h':
         ...
         scaleUB = arg
    if (len(sparqlDir) == 0):
        print 'translateWatDivQueries.py -s <sparqlDir> -t <sqlDir> -u <scaleUB> iflen'
        sys.exit()
    if (len(sqlDir) == 0):
        sqlDir = "./";
    print 'Input Dir is "', sparqlDir
    print 'Output Dir is "', sqlDir
    print 'scaleUB is ', scaleUB
