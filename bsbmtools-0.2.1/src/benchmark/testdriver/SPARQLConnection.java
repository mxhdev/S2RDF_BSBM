...
import benchmark.testdriver.custom.BSBMTranslateHelper;
import benchmark.testdriver.custom.OutputWriter;
...
public class SPARQLConnection implements ServerConnection{
	...
	private void executeQuery(String queryString, byte queryType, int queryNr, QueryMix queryMix) {
		...
		final int runNr = queryMix.getRun();
		
		if (this.serviceURL.startsWith(BSBMTranslateHelper.FP_PREFIX)) {
			
			// write to file

			final boolean storeQuery = (BSBMTranslateHelper.INCLUDE_WARMUPS || runNr >= 0);
			
			if (storeQuery) {
				BSBMTranslateHelper.storeQuery(logger, this.serviceURL, runNr, queryNr, queryString);
			}
			queryMix.setCurrent(0, 0.0);
			return;
		}
