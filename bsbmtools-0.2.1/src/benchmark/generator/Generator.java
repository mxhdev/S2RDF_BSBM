...
public class Generator {
	...
	//to shorten predicates (Changes Max & Timo)
	private static int shortenPredicates = 0;
	...
	private static Serializer getSerializer(String type) {
		...
		else if(t.equals("tsv"))
			return new TsvTriples(outputFileName, forwardChaining, nrOfOutputFiles, shortenPredicates);
		...
		while(i<args.length) {
			try {
				...
				// Changes Max & Timo
					...
					shortenPredicates = Integer.parseInt(args[i++ + 1]);
				}
				// End Changes
	...
	public static void printUsageInfos() {
		String output = "Usage:\n\n" +
						...
						"\t\t\tvirt (Virtuoso SQL dump), monetdb (SQL), xml (XML dump), tsv (tab separated)\n" +
						"\t\tdefault: nt\n" +
						"\t\tNote:\tBy chosing a named graph output format like TriG,\n\t\t\ta named graph model gets generated.\n" +
						"\t-sp <shorten predicates>\n" +
						"\t\tdefault: 0 (predicates won't be shorten, number specifies the last n characters of each predicate) \n" +
						"\t-pc <product count>\n" +
						"\t\tdefault: 100\n" +
