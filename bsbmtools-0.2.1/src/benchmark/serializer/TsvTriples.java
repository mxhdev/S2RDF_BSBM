...
public class TsvTriples implements Serializer {
	private FileWriter[] fileWriter;
	private boolean forwardChaining;
	private long nrTriples;
	private int currentWriter = 0;
	private int shortenPredicates = 0;
	
	public TsvTriples(String file, boolean forwardChaining, int shortenPredicates)
	{
		this(file, forwardChaining, 1, shortenPredicates);
	}
	
	public TsvTriples(String file, boolean forwardChaining, int nrOfOutputFiles, int shortenPredicates)
	{
		...
		try{
			...
			if(nrOfOutputFiles==1)
				fileWriter[0] = new FileWriter(file + ".tsv");
			else
				for(int i=1;i<=nrOfOutputFiles;i++)
					fileWriter[i-1] = new FileWriter(file + String.format(formatString, i) + ".tsv");
		} catch(IOException e){
		...
		this.shortenPredicates = shortenPredicates;
		nrTriples = 0l;
	}
	...
	private String createTriple(String subject, String predicate, String object)
	{
		...
		result.append("\t");
		if(shortenPredicates != 0){
			predicate = predicate.substring(predicate.length() - shortenPredicates);
