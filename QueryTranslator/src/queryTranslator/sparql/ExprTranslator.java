...
public class ExprTranslator implements ExprVisitor {
    ...
    public void visit(ExprFunction2 func) {
        ...
        if (operator.equals(Tags.NO_SUPPORT)) {
        	...
        	// Änderungen Max & Timo
        	String leftNew = left; 
        	String rightNew = right; 
        	
        	if(rightNew.contains("^^")){
        		rightNew = rightNew.substring(1, rightNew.length() - 1);
        		rightNew = rightNew.substring(0, rightNew.lastIndexOf("^^"));
        	}
        	
        	if(right.contains("^^xsd:dateTime")){
        		rightNew = rightNew.replace("T", " ");
        	}
        	
        	leftNew = leftNew.replace("\"'", "'").replace("'\"", "'");
        	rightNew = rightNew.replace("\"", "'");
        	
        	stack.push("(" + leftNew + operator + rightNew + ")");
        	// Ende Änderungen
        }
    }
