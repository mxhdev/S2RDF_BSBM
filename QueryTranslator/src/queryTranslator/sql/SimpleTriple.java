...
public class SimpleTriple implements SqlTriple{
	...
	public SqlStatement translate() {
		...
		if (predicate.isURI()) {
			this._tableName = FmtUtils.stringForNode(predicate, this._prefixMapping).replaceAll("[:]|[#]|[-]|[/]|[.]", "_").replaceAll("[<]|[>]", "");
		...
		if(predicate.isVariable()){
			...
			//For long predicates the following line has to be removed!!!
			String tableNameShort = this._tableName.substring(this._tableName.length() -19);
			//select.setFrom(this._tableName);
			select.setFrom(tableNameShort);
		}
		// ENDE ÄNDERUNGEN	
		// WHERE
